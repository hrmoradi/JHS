...
def modeling(covariates_all,outcomes):
    ...
    jhs_analysis = pd.read_csv(dataLocation + "Data_set" + ".csv") # Data_set_hf_rfImpute-True # _rfImpute-True # Data_set_hf_10y-False # Data_set_hf
    ...
    mid_dim = hp.choice('encode_dim', [2,3,5,7,10,20,25,30,40,50,70])
    ...
    # loss =  param_grid['']
    # optimizer = param_grid['']
    param_grid_ae = dict(num_layers=num_layers,mid_dim=mid_dim,
    ...
    tpe_best = []
    model = []
    print('glob_x_train.shape 3 ', glob_x_train.shape)
    if optimize_ae:
        ...
        print_log("Best: ", tpe_best, getXfromBestModelfromTrials(tpe_trials, 'acc'), param_grid_ae)
        ...
        param_grid_ae = {'batch_norm': False, 'batch_size': 32, 'dropout': 0.2, 'encode_dim': 2, 'epochs': 256, 'learning_rate': 0.01, 'num_layers': 1, 'num_neurons': 100}
        # {'batch_norm': False, 'batch_size': 4, 'dropout': 0.1, 'encode_dim': 70, 'epochs': 64, 'learning_rate': 0.001, 'num_layers': 1, 'num_neurons': 150}
    final_model_ae =  True
